...
function testParseJson(test) {
    var json = Json.parse(fs.readFileSync("test/parser_test.json", { encoding: 'utf8' }));
...
function parseBasicJson(test) {
    //reading the data using our custom js-yaml parser
    var json = Json.parse(fs.readFileSync("test/basic.json"));
    console.log(json.toJSON());
    test.done();
}
exports.parseBasicJson = parseBasicJson;
function testParseYaml(test) {
    var json = Json.parse_yaml(fs.readFileSync("test/complicated.yaml", { encoding: 'utf8' }));
    //console.log(JSON.stringify(json, null, 4));
    test.equal(json.start.row(), 1);
    test.equal(json.start.col(), 1);
